 ...
 */
export async function calculateIC(topic: string, subject?: string): Promise<number> {
  try {
    ...
    let ic = 0;
    if (percentage >= 50) ic = 10;
    else if (percentage >= 30) ic = 9;
    else if (percentage >= 20) ic = 8;
    else if (percentage >= 15) ic = 7;
    else if (percentage >= 10) ic = 6;
    else if (percentage >= 7) ic = 5;
    else if (percentage >= 5) ic = 4;
    else if (percentage >= 3) ic = 3;
    else if (percentage >= 1) ic = 2;
    else if (percentage > 0) ic = 1;
...
export interface TopicGap {
  ...
  frequency: number;
  cardsCount: number;
  gap: number; // Diferença entre IC e cards disponíveis
}
 ...
 */
export async function getTopicGaps(
  subject?: string,
  minIC: number = 7
): Promise<TopicGap[]> {
  try {
