 ...
 * Tipo: banca
 */
export async function harvestIdecan(): Promise<HarvestItem[]> {
  console.log('[Idecan] Iniciando coleta...');
  ...
  try {
    ...
    $('a[href*="concurso"], a[href*="edital"], article, .card').each((_, el) => {
      ...
      if (!items.find(i => i.url === fullUrl)) {
        items.push({ url: fullUrl, title, content: text.slice(0, 500), meta: { fonte: 'Idecan', vagas, salario } });
      }
    });
  } catch (e:any) {
    console.error('[Idecan] Erro:', e.message);
  }
  console.log(`[Idecan] Coletados ${items.length} itens`);
  return items;
}
